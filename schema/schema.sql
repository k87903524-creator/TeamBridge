...
CREATE TABLE EMPLOYEE (
    ...
    BIRTH_DATE      DATE         NULL,                   -- 본인이 마이페이지에서 입력, 이번 달 생일자 조회용
...
CREATE TABLE CALENDAR_EVENT (
    ...
    DEPT_ID        INT          NULL,       -- TEAM 일정만 등록 시점 부서 ID 저장
    IS_HOLIDAY     TINYINT(1)   NOT NULL DEFAULT 0,  -- 공휴일 여부
    CREATED_AT     DATETIME     NOT NULL DEFAULT NOW(),
...
CREATE TABLE APPROVAL (
    ...
    APPROVAL_STATUS  VARCHAR(10)  NOT NULL DEFAULT 'PROGRESS',  -- PROGRESS / APPROVED / REJECTED / WITHDRAWN
    CREATED_AT       DATETIME     NOT NULL DEFAULT NOW(),
...
-- ⑭ APPROVAL_FILE — APPROVAL 참조
-- ARCHIVE_FILE과 동일 구조: 지출결의서·프로젝트품의서 첨부용
...
-- ⑯ CHAT_MESSAGE — CHAT_ROOM, EMPLOYEE 참조
-- CHAT_ROOM_MEMBER보다 먼저 생성해야 LAST_READ_MESSAGE_ID FK를 바로 선언할 수 있음
...
CREATE TABLE CHAT_ROOM_MEMBER (
    ...
    ROOM_ID              INT      NOT NULL,
    EMPLOYEE_ID          INT      NOT NULL,
    LAST_READ_MESSAGE_ID INT      NULL,      -- 마지막으로 읽은 메시지, 아직 없으면 NULL
    ...
    CONSTRAINT FK_ROOMMEMBER_EMP        FOREIGN KEY (EMPLOYEE_ID)
                                      REFERENCES EMPLOYEE(EMPLOYEE_ID),
    CONSTRAINT FK_ROOMMEMBER_LASTREAD   FOREIGN KEY (LAST_READ_MESSAGE_ID)
...
-- ⑲ CHAT_ROOM_MEMBER_SUSPENSION — 채팅방에서 숨긴 메시지 구간 기록
-- 한 참여자가 나갔다가 다시 들어오는 등의 경우를 메시지 구간별로 보관
CREATE TABLE CHAT_ROOM_MEMBER_SUSPENSION (
    ...
    CONSTRAINT PK_CHAT_ROOM_MEMBER_SUSPENSION PRIMARY KEY (SUSPENSION_ID),
